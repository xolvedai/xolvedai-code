...
const tools = [
  ...
  { type: "function" as const, function: { name: "list_files", description: "List directory contents", parameters: { type: "object", properties: { path: { type: "string" } } } } },
  { type: "function" as const, function: { name: "search_codebase", description: "Search code across workspace", parameters: { type: "object", properties: { query: { type: "string" } }, required: ["query"] } } },
  { type: "function" as const, function: { name: "edit_file", description: "Edit or create a file (requires approval)", parameters: { type: "object", properties: { path: { type: "string" }, newContent: { type: "string" } }, required: ["path", "newContent"] } } },
  { type: "function" as const, function: { name: "run_terminal", description: "Run a terminal command (requires approval)", parameters: { type: "object", properties: { command: { type: "string" } }, required: ["command"] } } },
  { type: "function" as const, function: { name: "get_context", description: "Get current file and selection", parameters: { type: "object", properties: {} } } }
];

async function executeTool(name: string, args: any, panel: vscode.WebviewPanel): Promise<string> {
  ...
  try {
    switch (name) {
      ...
      case 'get_context': {
        ...
        if (!editor) return 'No active editor open';
